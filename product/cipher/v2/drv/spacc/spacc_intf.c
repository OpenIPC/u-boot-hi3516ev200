...
HI_S32 DRV_CIPHER_Init(HI_VOID)
{
    ...
    crypto_memset(&s_stSpaccEnv, sizeof(s_stSpaccEnv), 0, sizeof(SPACC_ENV_S));
    crypto_memset(&s_stSymcChn, sizeof(s_stSymcChn), 0, sizeof(s_stSymcChn));
    crypto_memset(&s_stDigestChn, sizeof(s_stDigestChn), 0, sizeof(s_stDigestChn));
...
static HI_S32 DRV_CIPHER_ConfigChn(HI_U32 softChnId,  CIPHER_CONFIG_CTRL_EX *pConfig)
{
    ...
    crypto_memset(&stChangeFlags, sizeof(stChangeFlags), 0, sizeof(stChangeFlags));
    stChangeFlags.bit1IV = pstChn->stCtrl.stChangeFlags.bit1IV;

    crypto_memset(u32IV, sizeof(u32IV), 0, sizeof(u32IV));
    crypto_memcpy(&pstChn->stCtrl,sizeof(pstChn->stCtrl), pConfig, sizeof(CIPHER_CONFIG_CTRL_EX));
    ...
    if( pConfig->enWorkMode == HI_UNF_CIPHER_WORK_MODE_CCM)
    {
         ...
         */
        pu8Buf = (HI_U8*)u32IV;
        crypto_memset(pu8Buf, sizeof(u32IV), 0, 16);
        pu8Buf[0] = 14 - pConfig->u32IVLen; /*IV[0] = q - 1 = 15 - n -1*/
        crypto_memcpy(pu8Buf + 1, sizeof(u32IV) -1, pConfig->u32IV, pConfig->u32IVLen);
...
static HI_S32 DRV_CIPHER_SymcWaitDone(SPACC_SYMC_CHN_S *pstChn, HI_U32 u32TimeOut)
{
    ...
    ret = cipher_queue_wait_timeout(&pstChn->queue,  &pstChn->bSymcDone, u32TimeOut);
    if (ret <= 0)
    {
...
static HI_S32 DRV_CIPHER_DigestWaitDone(SPACC_DIGEST_CHN_S *pstChn)
{
    ...
    {
        ret = cipher_queue_wait_timeout(&pstChn->queue, &pstChn->bDigestDone, u32TimeOut);
        if (ret <= 0)
        {
...
static HI_S32 DRV_CIPHER_CCM_HeadFormat(SPACC_SYMC_CHN_S *pstChn, HI_U32 u32EncLen, HI_U8 u8B0[16], HI_U8 u8B1[16], HI_U32 *pu32B1Len)
{
    ...
    */
    ...
    u32Index++;
    crypto_memcpy(&pbuf[u32Index], AES_BLOCK_SIZE - u32Index,
                                  ...
                                  HI_U32 u32BS, HI_U32 u32Ctrl, HI_BOOL bSetLast)
{
    ...
    while(u32Offset < u32Len)
    {
        ...
        crypto_memset(u32Buf, sizeof(u32Buf), 0, sizeof(u32Buf));
        crypto_memcpy(u32Buf, sizeof(u32Buf), &pu8DI[u32Offset], u32Size);
        for(i=0; i<u32BS/4; i++)
        {
        ...
        if (pu8DO != HI_NULL)
        {
            ...
            crypto_memcpy(&pu8DO[u32Offset], u32Len, (HI_U8*)u32Buf, u32Size);
        }
...
static HI_S32 DRV_CIPHER_CpuEncPhy(CIPHER_DATA_S *pstCIData, HI_U32 u32BlockSize, HI_BOOL bIsDecrypt)
{
    ...
    {
        /*Format N and A*/
        crypto_memset(u8B, sizeof(u8B), 0, sizeof(u8B));
        ...
        if (u32Val > 16) /* A and a can't puts in one block*/
        {
            ...
            crypto_memcpy(u8B+16+u32Offset, sizeof(u8B) - 16 - u32Offset,
        ...
        else if (u32Val > 0) /* A and a can puts in one block*/
        {
            ...
            crypto_memcpy(u8B+16+u32Offset, sizeof(u8B) - 16 - u32Offset, pu8AadVir, pstChn->stCtrl.u32ALen);
...
static HI_S32 DRV_CIPHER_CCM_NA(SPACC_SYMC_CHN_S *pstChn, HI_U32 u32EncLen)
{
    ...
    u32Index = 0;
    crypto_memset(pbuf, SPACC_PAD_BUF_SIZE, 0, 32);
    ...
    if(count != 0)
    {
        ...
        /*Set zero*/
        crypto_memset(&pbuf[u32Index], SPACC_PAD_BUF_SIZE - u32Index, 0, 16);
...
static HI_S32 DRV_DIGEST_Config(CIPHER_HASH_DATA_S *pCipherHashData, SPACC_CTRL_EN *penCtrl)
{
    ...
    *penCtrl = (HI_U32)*penCtrl | SPACC_CTRL_HASH_IN_FIRST;
    *penCtrl = (HI_U32)*penCtrl | SPACC_CTRL_HASH_IN_LAST;
...
HI_S32 HI_DRV_CIPHER_GetHandleConfigEx(CIPHER_CONFIG_CTRL_EX *pstCipherConfig)
{
    ...
    crypto_memcpy(pstCipherConfig, sizeof(CIPHER_CONFIG_CTRL_EX), &pstChn->stCtrl, sizeof(pstChn->stCtrl));
    cipher_mutex_unlock(&g_SymcMutex);
