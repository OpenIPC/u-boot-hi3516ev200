               ...
               unsigned long entry_phy_addr, void *entry_via_addr)
{
    ...
    crypto_memset(&s_symc, sizeof(s_symc), 0, sizeof(s_symc));
    crypto_memset(&s_digest, sizeof(s_digest), 0, sizeof(s_digest));
    crypto_memset(entry_via_addr, spacc_get_node_list_size(),
...
int spacc_symc_gettag(unsigned int chn_num, unsigned char *tag)
{
    ...
    crypto_memcpy(tag, 16, info->entry_symc_out[last].tag, sizeof(info->entry_symc_out[last].tag));
