                             ...
                             HI_U8 *pu8Mask, HI_U32 u32MaskLen)
{
    ...
    crypto_memset(u8Cnt, 4, 0, 4);
    crypto_memset(u8Md, HASH_RESULT_MAX_LEN, 0, HASH_RESULT_MAX_LEN);
    crypto_memcpy(u8Seed, u32Seedlen, pu8Seed, u32Seedlen);
...
HI_S32 Mbedtls_GetRandomNumber(HI_VOID *param, HI_U8 *pu8Rand, size_t u32Size)
{
    ...
    crypto_memset(pu8Rand, u32Size, 0, u32Size);
                                  ...
                                  HI_U32 *pu32OutLen)
{
    ...
    *pu32OutLen = 0;
    ...
    /*set lHash*/
    crypto_memcpy(pu8DB, u32DBLen, pu8LHASH, u32HLen);

    /*set PS with 0x00*/
    crypto_memset(&pu8DB[u32HLen], u32DBLen - u32HLen, 0,
    ...
    /*set M*/
    crypto_memcpy(&pu8DB[u32DBLen - u32InLen], u32InLen, pu8Input, u32InLen);
                                 ...
                                 HI_U32 *pu32OutLen)
{
    ...
    *pu32OutLen = 0;
    ...
    pu8Output[u32Index++] = 0x00;
    crypto_memcpy(&pu8Output[u32Index], u32KeyLen - u32Index, pu8Input, u32InLen);
                               ...
                               HI_U32 *pu32OutLen)
{
    ...
    *(pu8EB++) = u8BT;                 /* Private Key BT (Block Type) */
    ...
    {
    case 0x00:
        crypto_memset(pu8EB, u32KeyLen - (HI_U32)(pu8EB - pu8Output), 0x00, u32PLen);
        break;
    case 0x01:
        crypto_memset(pu8EB, u32KeyLen - (HI_U32)(pu8EB - pu8Output), 0xFF, u32PLen);
        break;
    case 0x02:
    ...
    pu8EB += u32PLen;
    *(pu8EB++) = 0x00;
    crypto_memcpy(pu8EB, u32KeyLen - (HI_U32)(pu8EB - pu8Output), pu8Input, u32InLen);
    *pu32OutLen = u32KeyLen;
                                        ...
                                        HI_U32 *pu32OutLen)
{
    ...
    *p++ = RSA_SIGN;
    ...
    case HI_UNF_CIPHER_HASH_TYPE_SHA1:
        ...
        crypto_memset(p, u32BufLen, 0xFF, u32PadLen);
        p += u32PadLen;
        *p++ = 0;
        u32BufLen = u32KeyLen - (HI_U32)(p - pu8Output);
        crypto_memcpy(p, u32BufLen, ASN1_HASH_SHA1, 15);
        p += 15;
        u32BufLen = u32KeyLen - (HI_U32)(p - pu8Output);
        crypto_memcpy(p, u32BufLen, pu8Input, u32InLen);
        break;
    case HI_UNF_CIPHER_HASH_TYPE_SHA224:
        ...
        *p++ = 0;
        u32BufLen = u32KeyLen - (HI_U32)(p - pu8Output);
        crypto_memcpy( p, u32BufLen, ASN1_HASH_SHA224, 19);
        p += 19;
        u32BufLen = u32KeyLen - (HI_U32)(p - pu8Output);
        crypto_memcpy( p, u32BufLen, pu8Input, u32InLen);
        break;
    case HI_UNF_CIPHER_HASH_TYPE_SHA256:
        ...
        *p++ = 0;
        u32BufLen = u32KeyLen - (HI_U32)(p - pu8Output);
        crypto_memcpy( p, u32BufLen, ASN1_HASH_SHA256, 19);
        p += 19;
        u32BufLen = u32KeyLen - (HI_U32)(p - pu8Output);
        crypto_memcpy( p, u32BufLen, pu8Input, u32InLen);
        break;
    case HI_UNF_CIPHER_HASH_TYPE_SHA384:
        ...
        *p++ = 0;
        u32BufLen = u32KeyLen - (HI_U32)(p - pu8Output);
        crypto_memcpy( p, u32BufLen, ASN1_HASH_SHA384, 19);
        p += 19;
        u32BufLen = u32KeyLen - (HI_U32)(p - pu8Output);
        crypto_memcpy( p, u32BufLen, pu8Input, u32InLen);
        break;
     case HI_UNF_CIPHER_HASH_TYPE_SHA512:
        ...
        *p++ = 0;
        u32BufLen = u32KeyLen - (HI_U32)(p - pu8Output);
        crypto_memcpy( p, u32KeyLen, ASN1_HASH_SHA512, 19);
        p += 19;
        u32BufLen = u32KeyLen - (HI_U32)(p - pu8Output);
        crypto_memcpy( p, u32BufLen, pu8Input, u32InLen);
        break;
                                        ...
                                        HI_U32 *pu32OutLen)
{
    ...
    *pu32OutLen = u32KeyLen;
    ...
    crypto_memset(&stHashAttr, sizeof(stHashAttr), 0, sizeof(HI_UNF_CIPHER_HASH_ATTS_S));
    ...
    u32Index = 0;
    crypto_memset(pu8M, u32Mlen, 0x00, 8);
    u32Index+=8;
    crypto_memcpy(&pu8M[u32Index], u32Mlen - u32Index, pu8Input, u32InLen);
    u32Index+=u32InLen;
    crypto_memcpy(&pu8M[u32Index], u32Mlen - u32Index, u8Salt, u32SLen);
    u32Index+=u32SLen;
    stHashAttr.eShaType = enHashType;
    ret = HI_MPI_CIPHER_HashInit(&stHashAttr, &HashHandle)
    ...
    u32Index = 0;
    crypto_memset(&pu8MaskedDB[u32Index], u32KeyLen - u32Index, 0x00, u32KeyLen - u32SLen -u32HLen - 2);
    u32Index+=u32KeyLen - u32SLen -u32HLen - 2;
    pu8MaskedDB[u32Index++] = 0x01;
    crypto_memcpy(&pu8MaskedDB[u32Index], u32KeyLen - u32Index, u8Salt, u32SLen);
                                  ...
                                  HI_U32 *pu32OutLen)
{
    ...
    *pu32OutLen = 0;
    ...
    for(i=u32HLen; i < u32KeyLen - u32HLen - 1; i++)
    {
        if((pu8DB[i] == 0x01))
        {
            crypto_memcpy(pu8Output, u32KeyLen, pu8DB+i+1, u32KeyLen - u32HLen - i - 2);
                                          ...
                                          HI_U32 *pu32OutLen)
{
    ...
    for( ; u32Index < u32KeyLen; u32Index++)
    {
        ...
        {
            crypto_memcpy(pu8Output, u32KeyLen, &pu8Input[u32Index+1], u32KeyLen - 1 - u32Index);
                               ...
                               HI_U32 *pu32OutLen)
{
    ...
    *pu32OutLen = pu8Input + u32KeyLen - pu8EB;
    crypto_memcpy(pu8Output, u32KeyLen, pu8EB, *pu32OutLen);
                                        ...
                                        HI_U32 *pu32OutLen)
{
    ...
    *pu32OutLen = u32HLen;
    ...
    case HI_UNF_CIPHER_HASH_TYPE_SHA1:
        ...
        {
            crypto_memcpy(pu8Output, u32KeyLen, p + 15, u32HLen);
            ret = HI_SUCCESS;
        }
        break;
    case HI_UNF_CIPHER_HASH_TYPE_SHA224:
        ...
        {
            crypto_memcpy(pu8Output, u32KeyLen, p + 19, u32HLen);
            ret = HI_SUCCESS;
        }
        break;
    case HI_UNF_CIPHER_HASH_TYPE_SHA256:
                                            ...
                                            HI_U8 *pu8MHash)
{
    ...
    crypto_memcpy(u8Salt, sizeof(u8Salt), &pu8MaskedDB[u32Index], u32SLen);
    ...
    crypto_memset(pu8M, u32MLen, 0x00, 8);
    u32Index+=8;
    crypto_memcpy(&pu8M[u32Index], u32MLen - u32Index, pu8MHash, u32HLen);
    u32Index+=u32HLen;
    crypto_memcpy(&pu8M[u32Index], u32MLen - u32Index, u8Salt, u32SLen);
    u32Index+=u32SLen;
    stHashAttr.eShaType = enHashType;
    ...
    ret = crypto_memcmp(u8H, pu8Maskedseed, u32HLen);
    if (ret != 0)
    {
                                  ...
                                  HI_U8 *pu8Output, HI_U32 *pu32OutLen)
{
    ...
    crypto_memset(u8EM, sizeof(u8EM), 0, sizeof(u8EM));
    ...
    case HI_UNF_CIPHER_RSA_ENC_SCHEME_NO_PADDING:
        ...
        *pu32OutLen = u32KeyLen;
        crypto_memcpy(u8EM+(u32KeyLen - u32InLen),
                                   ...
                                   HI_U8 *pu8Output, HI_U32 *pu32OutLen)
{
    ...
    case HI_UNF_CIPHER_RSA_ENC_SCHEME_NO_PADDING:
        *pu32OutLen = u32InLen;
        crypto_memcpy(pu8Output, u32KeyLen, u8EM, u32KeyLen);
        ret = HI_SUCCESS;
        break;
    case HI_UNF_CIPHER_RSA_ENC_SCHEME_BLOCK_TYPE_0:
    case HI_UNF_CIPHER_RSA_ENC_SCHEME_BLOCK_TYPE_1:
