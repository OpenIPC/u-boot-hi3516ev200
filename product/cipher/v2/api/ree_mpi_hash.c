...
static HI_U32 HashMsgPadding(HI_U8 *pu8Msg, HI_U32 u32ByteLen, HI_U32 u32TotalLen, HI_U32 u8BlockSize)
{
    ...
    crypto_memset(&pu8Msg[u32ByteLen], HASH_BLOCK_SIZE * 2, 0, u32PaddingLen - 1 - 8);
...
static HI_S32 CIPHER_HashInit(HI_UNF_CIPHER_HASH_ATTS_S *pstHashAttr, HI_HANDLE *pHashHandle)
{
    ...
    crypto_memset(pstHashInfo, sizeof(HASH_INFO_S), 0, sizeof(HASH_INFO_S));
    crypto_memset(&stHashData, sizeof(stHashData), 0, sizeof(CIPHER_HASH_DATA_S));
    ...
    case HI_UNF_CIPHER_HASH_TYPE_HMAC_SHA384:
        ...
        crypto_memcpy(&pstHashInfo->u32ShaVal[0], 8, &u64H, 8);
        u64H = CPU_TO_BE64(SHA384_H1);
        crypto_memcpy(&pstHashInfo->u32ShaVal[2], 8, &u64H, 8);
        u64H = CPU_TO_BE64(SHA384_H2);
        crypto_memcpy(&pstHashInfo->u32ShaVal[4], 8, &u64H, 8);
        u64H = CPU_TO_BE64(SHA384_H3);
        crypto_memcpy(&pstHashInfo->u32ShaVal[6], 8, &u64H, 8);
        u64H = CPU_TO_BE64(SHA384_H4);
        crypto_memcpy(&pstHashInfo->u32ShaVal[8], 8, &u64H, 8);
        u64H = CPU_TO_BE64(SHA384_H5);
        crypto_memcpy(&pstHashInfo->u32ShaVal[10], 8, &u64H, 8);
        u64H = CPU_TO_BE64(SHA384_H6);
        crypto_memcpy(&pstHashInfo->u32ShaVal[12], 8, &u64H, 8);
        u64H = CPU_TO_BE64(SHA384_H7);
        crypto_memcpy(&pstHashInfo->u32ShaVal[14], 8, &u64H, 8);
        break;
...
static HI_S32 CIPHER_HashUpdateBlock(HASH_INFO_S *pstHashInfo, HI_U8 *pu8InputData, HI_U32 u32InputDataLen)
{
    ...
    crypto_memset(&stHashData, sizeof(stHashData), 0, sizeof(stHashData));
    stHashData.enShaType = pstHashInfo->enShaType;
    stHashData.u32HardChn = SPACC_HASH_CHN;
    crypto_memcpy(&stHashData.u32ShaVal, sizeof(stHashData.u32ShaVal),
    ...
    crypto_memcpy(pstHashInfo->u32ShaVal, sizeof(pstHashInfo->u32ShaVal),
...
static HI_S32 CIPHER_HashUpdate(HI_HANDLE hCIHashHandle, HI_U8 *pu8InputData, HI_U32 u32InputDataLen)
{
    ...
    crypto_memset(u8DataPhy, HASH_MAX_BLOCK_SIZE, 0, HASH_MAX_BLOCK_SIZE);
    ...
    {
        crypto_memcpy(pstHashInfo->u8LastBlock+pstHashInfo->u8LastBlockSize, HASH_BLOCK_SIZE,
    ...
    if ( 0 < pstHashInfo->u8LastBlockSize )
    {
        ...
        crypto_memcpy(u8DataPhy, pstHashInfo->u8BlockSize, pstHashInfo->u8LastBlock, pstHashInfo->u8BlockSize);
    ...
    /* save tail data */
    crypto_memset(pstHashInfo->u8LastBlock, HASH_MAX_BLOCK_SIZE, 0, HASH_MAX_BLOCK_SIZE);
    pstHashInfo->u8LastBlockSize = u32InputDataLen;
    crypto_memcpy(pstHashInfo->u8LastBlock, HASH_BLOCK_SIZE, pu8InputData, u32InputDataLen);
...
static HI_S32 CIPHER_HashFinal(HI_HANDLE hCIHashHandle, HI_U8 *pu8OutputHash)
{
    ...
    crypto_memset(&stHashData, sizeof(CIPHER_HASH_DATA_S), 0, sizeof(CIPHER_HASH_DATA_S));
    ...
    crypto_memcpy(u8DataPhy, u32Tmp, pstHashInfo->u8LastBlock, u32Tmp);
    ...
    crypto_memcpy(pu8OutputHash, HASH_RESULT_MAX_LEN,
...
static HI_S32 CIPHER_HmacKeyInit(HI_UNF_CIPHER_HASH_ATTS_S *pstHashAttr, HI_U8 au8Hmackey[HASH_MAX_BLOCK_SIZE])
{
        ...
        || (pstHashAttr->eShaType == HI_UNF_CIPHER_HASH_TYPE_HMAC_SHA512))
    {
        ...
        {
            crypto_memcpy(au8Hmackey, HASH_MAX_BLOCK_SIZE, pstHashAttr->pu8HMACKey, pstHashAttr->u32HMACKeyLen);
            return HI_SUCCESS;
        }
    }
    else
...
static HI_S32 CIPHER_HmacInit(HI_UNF_CIPHER_HASH_ATTS_S *pstHashAttr, HI_HANDLE *pHashHandle)
{
    ...
    crypto_memcpy(pstHashInfo->u8Mac, sizeof(pstHashInfo->u8Mac), key_pad, pstHashInfo->u8BlockSize);
...
static HI_S32 CIPHER_HmacFinal(HI_HANDLE hCIHashHandle, HI_U8 *pu8Output)
{
    ...
    crypto_memset(hash_sum_1, HASH_RESULT_MAX_LEN, 0, HASH_RESULT_MAX_LEN);
    crypto_memset(&stHashAttr, sizeof(stHashAttr), 0, sizeof(HI_UNF_CIPHER_HASH_ATTS_S));
    stHashAttr.eShaType = pstHashInfo->enShaType;
    crypto_memcpy(key_pad, HASH_MAX_BLOCK_SIZE, pstHashInfo->u8Mac, pstHashInfo->u8BlockSize);
