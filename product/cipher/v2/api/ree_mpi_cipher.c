                                  ...
                                  const HI_UNF_CIPHER_ATTS_S *pstCipherAttr)
{
    ...
    crypto_memset(&stCIHandle, sizeof(stCIHandle), 0, sizeof(stCIHandle));
    crypto_memcpy(&stCIHandle.stCipherAtts, sizeof(stCIHandle.stCipherAtts),
...
HI_S32 HI_MPI_CIPHER_ConfigHandle(HI_HANDLE hCipher, HI_UNF_CIPHER_CTRL_S* pstCtrl)
{
    ...
    crypto_memset(&configdata, sizeof(configdata), 0, sizeof(CIPHER_CONFIG_CTRL_EX));

    crypto_memcpy(&configdata.u32Key, sizeof(configdata.u32Key), pstCtrl->u32Key, 32);
    crypto_memcpy(&configdata.u32IV, sizeof(configdata.u32IV), pstCtrl->u32IV, 16);
...
HI_S32 HI_MPI_CIPHER_ConfigHandleEx(HI_HANDLE hCipher, HI_UNF_CIPHER_CTRL_EX_S* pstExCtrl)
{
    ...
    */
    ...
    case HI_UNF_CIPHER_ALG_DES:
    {
        ...
        crypto_memcpy(&configdata.u32Key, sizeof(configdata.u32Key),
            pstDesCtrl->u32Key, sizeof(pstDesCtrl->u32Key));
        crypto_memcpy(&configdata.u32IV, sizeof(configdata.u32IV),
    ...
    case HI_UNF_CIPHER_ALG_AES:
    {
        ...
        else
        {
            ...
            crypto_memcpy(&configdata.u32OddKey, sizeof(configdata.u32OddKey),
    ...
    case HI_UNF_CIPHER_ALG_SM1:
    {
        ...
        crypto_memcpy(&configdata.u32Key[0], sizeof(HI_U32) * 4,
            pstSm1Ctrl->u32EK, sizeof(pstSm1Ctrl->u32EK));
        crypto_memcpy(&configdata.u32Key[4], sizeof(HI_U32) * 4,
            pstSm1Ctrl->u32AK, sizeof(pstSm1Ctrl->u32AK));
        crypto_memcpy(&configdata.u32Key[8], sizeof(HI_U32) * 4,
...
HI_S32 HI_MPI_CIPHER_EncryptMulti(HI_HANDLE hCipher, HI_UNF_CIPHER_DATA_S *pstDataPkg, HI_U32 u32DataPkgNum)
{
    ...
    crypto_memcpy(pstPkg, u32Len, pstDataPkg, u32Len);
...
HI_S32 HI_MPI_CIPHER_GetHandleConfig(HI_HANDLE hCipherHandle, HI_UNF_CIPHER_CTRL_S* pstCtrl)
{
    ...
    crypto_memcpy(pstCtrl->u32Key, sizeof(pstCtrl->u32Key),
        &configdata.u32Key, 32);
    crypto_memcpy(pstCtrl->u32IV, sizeof(pstCtrl->u32IV),
...
HI_S32 HI_MPI_CIPHER_GetTag(HI_HANDLE hCipher, HI_U8 *pu8Tag, HI_U32 *pu32TagLen)
{
    ...
    crypto_memset(&stTag, sizeof(CIPHER_TAG_S), 0, sizeof(CIPHER_TAG_S));
    ...
    if (Ret == HI_SUCCESS)
    {
        ...
        crypto_memcpy(pu8Tag, *pu32TagLen, stTag.u32Tag, stTag.u32TagLen);
