...
int start_gx(unsigned int layer, unsigned long addr, unsigned int strd, hi_vo_rect gx_rect)
{
    ...
    if ((vo_intf_type == VO_INTF_HDMI) ||
        ...
        vo_drv_is_lcd_intf(vo_intf_type)) {
...
hi_void video_layer_set_zme_cfg(unsigned int layer, rect *disp_rect)
{
...
hi_void video_layer_set_csc_by_intf(unsigned int layer, vo_intf_type vo_intf_type)
{
    vo_layer vo_layer;
