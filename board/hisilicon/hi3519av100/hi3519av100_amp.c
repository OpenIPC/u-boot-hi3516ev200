...
#if defined(CONFIG_HIFMC_SPI_NAND)|| defined(CONFIG_HIFMC_NAND)              /* spi nand configure */
...
static struct boot_medium_interface boot_intf[1]= {0};
...
void boot_medium_init(void)
{
    ...
    if(get_boot_media()==BOOT_MEDIA_SPIFLASH)
    {
        ...
        boot_intf[0].init=NULL;
#ifdef CONFIG_HIFMC_SPI_NOR
        boot_intf[0].read=spinor_flash_read;
#endif
        strncpy(boot_intf[0].name,"spinor",sizeof(boot_intf[0].name));
    }
    else if(get_boot_media()==BOOT_MEDIA_NAND)
    {
        ...
        boot_intf[0].read=nand_flash_read;
#endif
        strncpy(boot_intf[0].name,"nand",sizeof(boot_intf[0].name));
    }
    else if(get_boot_media()==BOOT_MEDIA_EMMC)
    {
        ...
        boot_intf[0].init=NULL;
#ifdef CONFIG_MMC
        boot_intf[0].read=mmc_read_data;
#endif
        strncpy(boot_intf[0].name,"emmc",sizeof(boot_intf[0].name));
    }
    else
    {
...
int misc_init_r(void)
{
    ...
    if (auto_update_flag) {
		...
		if(0 == result) {
			...
			udelay(10000);
			if ((uregval & UPGRADE_STATUS_PROCESSING) != 0) {
