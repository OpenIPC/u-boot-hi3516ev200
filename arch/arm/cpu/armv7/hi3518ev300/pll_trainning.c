...
static void svb_voltage_regulator_init(struct voltage_regulator *regulator)
{
	...
	val = readl(regulator->reg_virt + SVB_SYS_VOLTAGE_OFFSET + 0x20 * regulator->pwm_sel);
	...
	if (regulator->curr + step_25 < regulator->max){
		regulator->steps[1] = regulator->curr + step_25;
	} else {
	...
	if (regulator->curr > regulator->min + step_25){
		regulator->steps[3] = regulator->curr - step_25;
	} else {
	...
	if (regulator->curr > regulator->min + step_80){
		regulator->steps[5] = regulator->curr - step_80;
	} else {
		regulator->steps[5] = regulator->min;
	}

	if (regulator->curr > regulator->min + step_100){
		regulator->steps[6] = regulator->curr - step_100;
	} else {
		regulator->steps[6] = regulator->min;
	}

	if(regulator->steps[5] <= regulator->steps[6]){
		regulator->steps[6] = 0xffffffff;
	}
	
	if (regulator->steps[4] <= regulator->steps[5]){
		regulator->steps[5] = 0xffffffff;
	}
...
static int svb_voltage_adjust(struct voltage_regulator *regulator, int step)
{
	...
	writel(val, regulator->reg_virt + SVB_SYS_VOLTAGE_OFFSET + 0x20 * regulator->pwm_sel);
	writel(5, regulator->reg_virt + SVB_SYS_VOLTAGE_UPDATE_OFFSET + 0x20 * regulator->pwm_sel);
	return 0;
}

static void svb_voltage_restore(struct voltage_regulator *regulator)
{
	unsigned int val = readl(regulator->reg_virt + SVB_SYS_VOLTAGE_OFFSET + 0x20 * regulator->pwm_sel);
	val &= 0xffffff00;
	val |= regulator->curr;
	writel(val, regulator->reg_virt + SVB_SYS_VOLTAGE_OFFSET + 0x20 * regulator->pwm_sel);
	writel(5, regulator->reg_virt + SVB_SYS_VOLTAGE_UPDATE_OFFSET + 0x20 * regulator->pwm_sel);
